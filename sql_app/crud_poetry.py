...
def get_poetry_by_author_id(db: Session, author_id: str, skip: int = 0, limit: int = 5):
...
def get_poetry_by_type_and_name_and_author(db: Session, poetry_type: str, name: str, author_id: str):
    return db.query(Poetry).filter(Poetry.type == poetry_type, Poetry.name == name, Poetry.author_id == author_id,
                                   Poetry.is_delete == 0).first()
...
def get_poetry_by_type_random(db: Session, poetry_type: str):
    return db.query(Poetry).filter(Poetry.type == poetry_type, Poetry.phrase != "", Poetry.is_delete == 0).order_by(
        func.random()).first()
...
def update_author(db: Session, author_id, introduce):
