...
@router.put("/edit/{user_id}", summary="编辑用户", description="这是一个编辑用户接口")
async def edit_user(user_id: str, user_create: UserCreate, db: Session = Depends(get_db),
                    user: User = Depends(get_current_user)):
    db_user = crud_users.get_user(db, user_id=user_id)
    if db_user is None:
        raise exception.NotExitException(name=f"ID {user_id}")
    ...
    if user_create.hashed_password:
        field_check.check_password(user_create.hashed_password)
    db_name = crud_users.get_user_by_name_edit(db, name=user_create.name, user_id=user_id)
    db_email = crud_users.get_user_by_email_edit(db, email=user_create.email, user_id=user_id)
    if db_name:
        raise exception.AlreadyExistException(name="name")
    if db_email:
        raise exception.AlreadyExistException(name="email")
    crud_users.edit_user(db=db, user=user_create, user_id=user_id)
    user_info = crud_users.get_user(db, user_id=user_id)
    return json_format(user_info)
