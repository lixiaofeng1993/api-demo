...
def handle_wx_text(data_list: list):
    ...
    for data in data_list:
        content += f"<a href='weixin://bizmsgmenu?msgmenucontent={data.id}&msgmenuid=3'>{data.name}</a>\n"
    return content


def poetry_by_author_id(db: Session, request: Request, author_id: str, skip: int, content: str = ""):
    if not content:
    ...
    if data_list:
        ...
        content += ">>>点击古诗名字 "
        more_text = f" <a href='weixin://bizmsgmenu?msgmenucontent=AUTHOR-{author_id}&msgmenuid=4'>更多</a> "
...
def poetry_by_type(db: Session, request: Request, text: str, skip: int, val):
    ...
    content += handle_wx_text(data_list)
    content += ">>>点击古诗名字 "
    more_text = f" <a href='weixin://bizmsgmenu?msgmenucontent=POETRY_TYPE-{val}&msgmenuid=5'>更多</a> "
    content += "或者查看" + more_text if len(data_list) == 10 else ""
...
def author_by_dynasty(db: Session, request: Request, text: str, skip: int, val):
    ...
    content += handle_wx_text(data_list)
    more_text = f" <a href='weixin://bizmsgmenu?msgmenucontent=DYNASTY-{val}&msgmenuid=6'>更多</a> "
    content += ">>>点击诗人名字 "
    content += "或者查看" + more_text if len(data_list) == 10 else ""
...
def send_more(db: Session, request: Request, text: str, skip: str, content: str = ""):
    if "DYNASTY-" in text or "POETRY_TYPE-" in text or "AUTHOR-" in text or "RECOMMEND-" in text:
        ...
        if "RECOMMEND" in text:
            ...
            content = poetry_by_author_id(db, request, val, skip + 10)
    return content
    ...
    if skip:
        ...
        if data:
            ...
            if data:
                ...
                if data:
                    content = f"古诗名字：\n" + f"<a href='weixin://bizmsgmenu?msgmenucontent={data.id}&msgmenuid=2'>" \
                                           f"{data.name}</a>\n>>>点击古诗名字"
    return content
